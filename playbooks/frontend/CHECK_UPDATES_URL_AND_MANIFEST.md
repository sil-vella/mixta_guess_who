...
- **Base URL:** From `API_URL` (compile-time `--dart-define=API_URL=...`).
  - **VPS APK (build_apk.sh default):** `https://guesswho.mixta.mt`
  - **Local build:** `http://10.0.2.2:8081` or value from env.

**Full URL (VPS):**
```text
https://guesswho.mixta.mt/public/check-updates?current_version=2.0.4
```
(No trailing slash; `current_version` is the app’s `PackageInfo.version`.)

**Code references:**
- Flutter: `mixta_flutter/lib/core/services/version_check_service.dart` → `route = '/public/check-updates?current_version=$currentVersion'`; URL = `Config.apiUrl` + route (`ConnectionsApiModule(baseUrl)` with `baseUrl` from `Config.apiUrl`).
- Backend: `mixta_flask/core/modules/system_actions_module/system_actions_main.py` → `_register_route_helper("/public/check-updates", self.check_updates, methods=["GET"])`.
