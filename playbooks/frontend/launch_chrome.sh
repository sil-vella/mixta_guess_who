...
# Navigate to Flutter project directory (repo root = playbooks/frontend/../..)
FLUTTER_PROJECT_DIR="${FLUTTER_PROJECT_DIR:-$REPO_ROOT/mixta_flutter}"
cd "$FLUTTER_PROJECT_DIR" || {
  echo "❌ Flutter project not found: $FLUTTER_PROJECT_DIR"
  exit 1
}

# Same path mixta_flask custom_logging uses (tools/logger/server.log)
SERVER_LOG_FILE="${SERVER_LOG_FILE:-$REPO_ROOT/mixta_flask/tools/logger/server.log}"
...
# Flutter may prepend text to each line; extract the tail that matches:
# [ISO8601] [INFO|DEBUG|WARNING|ERROR] [AppLogger] message
filter_logs() {
    # Bash 3.2–compatible: apply regex to a line that *starts* with the pattern (we strip prefixes below).
    # Dart ISO-8601 may include fractional seconds and optional trailing Z
    STRICT_PATTERN='^\[([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]+)?Z?)\] \[(INFO|DEBUG|WARNING|ERROR)\] \[AppLogger\] (.*)$'

    while IFS= read -r line; do
        # Pull the formatted segment from anywhere in the line (web / tooling prefixes)
        formatted=$(echo "$line" | grep -oE '\[[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]+)?Z?\] \[(INFO|DEBUG|WARNING|ERROR)\] \[AppLogger\].*' | head -1)
        [ -z "$formatted" ] && continue

        if [[ "$formatted" =~ $STRICT_PATTERN ]]; then
            ...
            # Same file as mixta_flask custom_logging / server tail
