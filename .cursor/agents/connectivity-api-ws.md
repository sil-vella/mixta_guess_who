...
For VPS-side changes (SSH, CORS on the server, nginx, Ansible playbooks, deployment, firewall, or inventory), use the **playbooks-only** subagent. When you conclude that the fix is on the server (e.g. CORS for `guesswho.mixta.mt`), suggest the user invoke playbooks-only to apply or inspect the relevant playbooks and server config.
